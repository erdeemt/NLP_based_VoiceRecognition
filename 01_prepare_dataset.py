...
N_MELS = 20
CONF_THRESHOLD = 0.50
MAX_WINDOWS = 20 # Maximum number of windows for fixed length

# SBERT Model Keys (4 different models will be tested)
SBERT_MODELS = {
    "TR": ["multi_1", "multi_2"], # Multilingual models
    "EN": ["multi_1", "multi_2"] # Original + Multilingual ##"en_orig_1", "en_orig_2"
}
...
def main():
    print("🚀PHASE 1: Confidence Score Vector Integration has been started...")
    
   # Load ASR Models
    print("📦 ASR Models are loading...")
    ...
    #embedder has started.
    embedder = ProjectEmbedder()
    
   # Creating a separate dataset for each BERT model
    for lang in ["EN"]: #TR
        print(f"\n{'='*60}")
     ...
     # Model selection
       ...
       # For each SBERT model
        for sbert_key in SBERT_MODELS[lang]:
            print(f"\n🔹 SBERT Model: {sbert_key}")
            ...
            for label_str in class_folders:
                ...
                # Filter only relevant language
                lang_files = [f for f in all_files if f.startswith(f"{lang}_")]
                
                if not lang_files:
                    continue
                
                print(f"  📁 Class {label_id}: Processing file {len(lang_files)}...")
                ...
                for wav_name in tqdm(lang_files, leave=False, desc=f"  Class {label_id}"):
                    try:
                        ...
                        for start in range(0, len(y) - win_samples, hop_samples):
                          ...
                          # Predictions without silence and passing the threshold
                            if cid != 0 and conf >= CONF_THRESHOLD:
                                window_confidences.append(conf)
                                predicted_commands.append(cid)
                        
                        # There must be at least 1 valid window
                        if len(window_confidences) == 0:
                            continue
                        
                        #1. TEXT EMBEDDING (SBERT)
                        txt = cmd_dict.get(label_id, "")
                        text_embedding = embedder.get_sbert_vector(txt, model_key=sbert_key)
                        
                        #2. CONFIDENCE SCORE VECTOR (PDF Requirement!)
                        # Bring to fixed length (padding/truncation)
                        conf_vector = pad_or_truncate(np.array(window_confidences), MAX_WINDOWS)
                        
                        # 3. CONCATENATION: X = [Text Embedding | Confidence Vector]
                        ...
                        print(f"    ⚠️ Even ({wav_name}): {e}")
                        continue
            
            #Save data
            if len(data_X) > 0:
                output_dir = f"datasets_phase1/{lang}/{sbert_key}"
                os.makedirs(output_dir, exist_ok=True)
                
                np.save(f"{output_dir}/X_features.npy", np.array(data_X))
                np.save(f"{output_dir}/y_labels.npy", np.array(data_Y))
                
                print(f"  ✅ Saved: {len(data_X)} example")
                print(f"     📊 X shape: {np.array(data_X).shape}")
                print(f"     🎯 Y shape: {np.array(data_Y).shape}")
            else:
                print(f"  ⚠️ No data found!")

    print(f"\n{'='*60}")
    print("🏁Phase 1 Tamamulandi!")
    print(f"{'='*60}")
    print("\n📁 Created Files:")
